...
def test_extract_european_returns_country_in():
    result = extract_country("european companies")
    assert "country_in" in result
    assert "de" in result["country_in"]
    assert "fr" in result["country_in"]
    assert "ro" in result["country_in"]
...
def test_extract_europe_list_is_sorted():
    result = extract_country("european companies")
    assert result["country_in"] == sorted(result["country_in"])


def test_extract_scandinavian():
    assert extract_country("scandinavian manufacturers") == {"country_in": ["dk", "fi", "no", "se"]}

def test_extract_nordic_same_as_scandinavian():
    assert extract_country("nordic startups") == {"country_in": ["dk", "fi", "no", "se"]}
